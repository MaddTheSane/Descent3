...
/// - Parameter libname: the path & filename of the HOG file
...
/// - Parameter handle: the handle returned by cf_OpenLibrary()
...
/// - parameter filename: the name if the file, with or without a path
/// - parameter mode: the standard C mode string
...
/// - Parameter cfp: the file pointer returned by cfopen()
int cfilelength(CFILE *cfp);

/// Closes an open CFILE.
/// - Parameter cfile: the file pointer returned by cfopen()
...
/// - Parameter buf: where the string is written
/// - Parameter n: the maximum string length, including the terminating 0
/// - Parameter cfp: the CFILE pointer
...
/// - Parameter buf: pointer to the string
/// - parameter cfp: the CFILE pointer
/// - Returns: the number of bytes written
/// Throws an exception of type (cfile_error *) if the OS returns an error on write
...
/// Same as cf_GetfileCRC, except works with CFILE pointers
