...
void MultiDoMSafeFunction(uint8_t *data) {
  ...
  case MSAFE_OBJECT_WORLD_POSITION:
    ...
    mstruct->pos.y = MultiGetFloat(data, &count);
    mstruct->pos.z = MultiGetFloat(data, &count);
    mstruct->orient.columns[2].x = MultiGetFloat(data, &count);
    mstruct->orient.columns[2].y = MultiGetFloat(data, &count);
    mstruct->orient.columns[2].z = MultiGetFloat(data, &count);
    mstruct->orient.columns[0].x = MultiGetFloat(data, &count);
    mstruct->orient.columns[0].y = MultiGetFloat(data, &count);
    mstruct->orient.columns[0].z = MultiGetFloat(data, &count);
    mstruct->orient.columns[1].x = MultiGetFloat(data, &count);
    mstruct->orient.columns[1].y = MultiGetFloat(data, &count);
    mstruct->orient.columns[1].z = MultiGetFloat(data, &count);
    break;
...
void MultiSendMSafeFunction(uint8_t type, msafe_struct *mstruct) {
  ...
  case MSAFE_OBJECT_WORLD_POSITION:
    ...
    MultiAddFloat(mstruct->pos.y, data, &count);
    MultiAddFloat(mstruct->pos.z, data, &count);
    MultiAddFloat(mstruct->orient.columns[2].x, data, &count);
    MultiAddFloat(mstruct->orient.columns[2].y, data, &count);
    MultiAddFloat(mstruct->orient.columns[2].z, data, &count);
    MultiAddFloat(mstruct->orient.columns[0].x, data, &count);
    MultiAddFloat(mstruct->orient.columns[0].y, data, &count);
    MultiAddFloat(mstruct->orient.columns[0].z, data, &count);
    MultiAddFloat(mstruct->orient.columns[1].x, data, &count);
    MultiAddFloat(mstruct->orient.columns[1].y, data, &count);
    MultiAddFloat(mstruct->orient.columns[1].z, data, &count);
    break;
  case MSAFE_OBJECT_SPARKS:
    sequence = NETSEQ_PLAYING;
    MultiAddUshort(mstruct->objhandle & HANDLE_OBJNUM_MASK, data, &count);
